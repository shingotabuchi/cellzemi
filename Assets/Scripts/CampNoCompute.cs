...
public class CampNoCompute : MonoBehaviour
{
    public enum Mode
    {
        C,
    ...
    public int DIM;
    public float initE,dt,D,gamma,a,dx,Cmin,Cmax,tau,tauar,taurr,eta,beta,alpha,rho;
    float dy;
    public int loopCount;
    public Mode mode = Mode.C;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < DIM; i++)
        {
            for (int j = 0; j < DIM; j++)
            {
                ...
                state[i + j*DIM] = 0;
                C[i + j*DIM] = 0;
                // if(Random.Range(0f,1f)<rho)C[i + j*DIM] = Cmin;
                // C[i + j*DIM] = Random.Range(0f,100f);
                // E[i + j*DIM] = 0;
                E[i + j*DIM] = initE;
                // E[i + j*DIM] = Random.Range(0f,1f);
                if(mode==Mode.state) plotPixels[i + j*DIM] = GrayScale(state[i + j*DIM]/2f);
                else if(mode == Mode.C)plotPixels[i + j*DIM] = GrayScale((C[i + j*DIM]-minC)/(maxC-minC));
                else if(mode == Mode.E)plotPixels[i + j*DIM] = GrayScale(E[i + j*DIM]);
            }
        }
    ...
    void Step()
    {
        ...
        for (int i = 0; i < DIM; i++)
        {
            for (int j = 0; j < DIM; j++)
            {
                ...
                else if(C[i + j*DIM]>=Cmin||Random.Range(0f,1f)<rho)
                // else if(Random.Range(0f,1f)<rho)
                {
                    state[i + j*DIM] = 1;
                }
                ...
                {
                    E[i + j*DIM] += (eta + beta*C[i + j*DIM]-alpha*E[i + j*DIM])*dt;
                    if(E[i + j*DIM]>1)  E[i + j*DIM] = 1f;
                }
        ...
        {
            maxC = Mathf.Max(maxC,maxCtmp);
            minC = Mathf.Min(minC,minCtmp);
        }
