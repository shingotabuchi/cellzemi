...
public class Sand : MonoBehaviour
{
    ...
    public int spawnSizeMax;
    public int spawnSizeMin;
    public float n;
    ...
    void Start()
    {
        ...
        compute.SetInt("spawnSize", 1);

        sandBuffer = new ComputeBuffer(DIM * DIM, Marshal.SizeOf(typeof(SandParticle)));
        particleTypeBuffer = new ComputeBuffer(DIM * DIM, sizeof(int));
        ...
        compute.Dispatch(Init, (DIM * DIM + 63) / 64, 1, 1);
    ...
    void Update()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            if (PosOnCanvas(touchPoint))
            {
                ...
                // float x = x1 - Mathf.Pow((Mathf.Pow(x1, (n + 1)) - Mathf.Pow(x0, (n + 1))) * y + Mathf.Pow(x0, (n + 1)), (1 / (n + 1)));
                // int spawnSize = (int)(Mathf.Max(1f, x + 1f));

                // if (spawnSize != 1) print(spawnSize);
                // compute.SetInt("spawnSize", spawnSize);
                // // if (Random.Range(0f, 1f) < bigProb)
                // //     {
                // //         compute.SetInt("spawnSize", 2);
                // //     }
                // //     else compute.SetInt("spawnSize", 1);
    ...
    void FixedUpdate()
    {
        for (int kk = 0; kk < loopCount; kk++)
        {
            ...
            compute.Dispatch(Step, (DIM * DIM + 63) / 64, 1, 1);
        }
        compute.Dispatch(Plot, (DIM + 7) / 8, (DIM + 7) / 8, 1);
        RenderTexture.active = renderTexture;
        plotTexture.ReadPixels(new Rect(0, 0, renderTexture.width, renderTexture.height), 0, 0);
